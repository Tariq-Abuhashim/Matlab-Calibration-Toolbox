function icub_calibrate_right(run_id)
% The file 
%
...
disp( 'Calibrating right images');

right_image_path = './data/Calib_right/images/right' ;
disp( ['right_image_path : ', right_image_path] );

calib_path = strcat( right_image_path, '/results/run_', num2str(run_id) ) ; mkdir( calib_path ) ;
disp( ['calib_path : ', calib_path] );

disp( 'Reading from image_path and then copying calibration images to copy_path ...' ) ;
copy_calib_images( right_image_path, calib_path ) ;
